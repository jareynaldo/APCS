...
public class Magpie4V2 {
    ...
    public String getResponse(String statement) {
        ...
        // Responses which require transformations
            ...
            response = transformIWantToStatementV2(statement);
        }

        else {
            // Look for a two word (you <something> me) pattern
            int position = findKeyword(statement, "you", 0);
            int positionV2 = findKeyword(statement, "I", 0);

            if (position >= 0 && findKeyword(statement, "me", position) >= 0) {
                response = transformYouMeStatement(statement);
            } else if (positionV2 >= 0 && findKeyword(statement, "you", position) >= 0) {
                response = transformYouMeStatementV2(statement);
            } else {
    ...
    private String transformIWantToStatementV2(String statement) {
        // Remove the final period, if there is one
        statement = statement.trim();
        String lastChar = statement.substring(statement.length() - 1);
     ...
     * @return the transformed statement
     */
    private String transformYouMeStatementV2(String statement) {
        // Remove the final period, if there is one
        statement = statement.trim();
        String lastChar = statement.substring(statement.length() - 1);
    ...
    private String getRandomResponse() {
        ...
        String response = "";
        String[] randomResponses = { "Interesting, tell me more.", "Hmmm.", "Do you really think so?",
                "You don't say.", "Very interesting.", "Please, tell me more." };
